...
struct App : public OpenGLApplication
{
    ...
    void init() override
    {
        ...
        grassTex_.load("../textures/grass.jpg");
        grassTex_.setWrap(GL_REPEAT);
        grassTex_.setFiltering(GL_LINEAR);
        grassTex_.enableMipmap();

        streetTex_.load("../textures/street.jpg");
        streetTex_.setWrap(GL_REPEAT);
        streetTex_.setFiltering(GL_LINEAR);
        streetTex_.enableMipmap();

        streetCornerTex_.load("../textures/streetcorner.jpg");
        streetCornerTex_.setWrap(GL_REPEAT);
        streetCornerTex_.setFiltering(GL_LINEAR);
        streetCornerTex_.enableMipmap();

        treeTex_.load("../textures/pine.jpg");
        treeTex_.setWrap(GL_REPEAT);
        treeTex_.setFiltering(GL_NEAREST); 

        lampTex_.load("../textures/streetlight.jpg");
        lampTex_.setWrap(GL_REPEAT);
        lampTex_.setFiltering(GL_NEAREST); 

        carTex_.load("../textures/car.png");
        carTex_.setWrap(GL_CLAMP_TO_EDGE);
        carTex_.setFiltering(GL_LINEAR);
        carTex_.enableMipmap();

        windowTex_.load("../textures/window.png");
        windowTex_.setWrap(GL_CLAMP_TO_EDGE);
        windowTex_.setFiltering(GL_NEAREST); 
        initShapeData();
        loadModels();
        initStaticMatrices();
    ...
    void loadShaderPrograms()
    {
        ...
        glUniform1i(glGetUniformLocation(transformSP_, "diffuseSampler"), 0);
        glUseProgram(0);
    ...
    {    
        ...
        for (unsigned int i = 0; i < N_STREET_PATCHES; ++i) {
            ...
            if (i < 7u * 4u) {
                bindTexture(streetTex_);
                street_.draw();
            }
            else {
                bindTexture(streetCornerTex_);
                streetcorner_.draw();
            }
        }
    }
...
private:
    ...
    Texture2D grassTex_;
    Texture2D streetTex_;
    Texture2D streetCornerTex_;
    Texture2D treeTex_;
    Texture2D lampTex_;
    Texture2D carTex_;
    Texture2D windowTex_;
